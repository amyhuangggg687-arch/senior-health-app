...
import { User, Users, Bell, PlayCircle, Star, Brain, ClipboardCheck } from 'lucide-react';
      ...
      {isSenior ? (
        <>
          ...
          <Link 
            ...
            className={cn(
              "flex flex-col items-center gap-1 transition-all",
              location.pathname === '/checkin' ? "text-vintage-orange scale-110" : "text-vintage-brown/30"
            )}
          >
            <ClipboardCheck className={cn("w-6 h-6", location.pathname === '/checkin' ? "fill-vintage-orange/20" : "")} />
            <span className="text-sm font-hand font-bold">每日打卡</span>
          </Link>
          <Link 
            to="/favorites" 
            className={cn(
