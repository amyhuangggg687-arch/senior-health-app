import { motion } from 'motion/react';
import { Star, Heart, Utensils, Bookmark, Trash2 } from 'lucide-react';
import { useFamily } from '../FamilyContext';
import { MOCK_VIDEOS } from '../data/mockData';
import { clsx, type ClassValue } from 'clsx';
import { twMerge } from 'tailwind-merge';

function cn(...inputs: ClassValue[]) {
  return twMerge(clsx(inputs));
}

export default function SeniorFavorites() {
  const { sharedItems, favoriteIds, toggleFavorite } = useFamily();
  
  const favoriteItems = sharedItems.filter(item => favoriteIds.includes(item.id));

  return (
    <div className="min-h-screen bg-vintage-texture pb-24">
      <header className="bg-white/80 backdrop-blur-sm px-6 py-10 shadow-sm border-b-4 border-vintage-brown/10">
        <h1 className="text-3xl font-hand font-bold text-vintage-brown flex items-center gap-3">
          <Star className="w-8 h-8 text-vintage-yellow fill-vintage-yellow" />
          我的健康菜谱
        </h1>
      </header>

      <main className="px-6 py-8">
        {favoriteItems.length === 0 ? (
          <div className="text-center py-20 bg-white/50 rounded-sm border-2 border-dashed border-vintage-brown/20">
            <div className="w-24 h-24 bg-vintage-cream rounded-full flex items-center justify-center mx-auto mb-6 border-2 border-vintage-brown/10">
              <Bookmark className="w-10 h-10 text-vintage-brown/30" />
            </div>
            <p className="text-vintage-brown/60 text-2xl font-hand">还没有收藏的菜谱哦</p>
            <p className="text-zinc-400 font-retro mt-2">在“孩子们”页面可以收藏推荐</p>
          </div>
        ) : (
          <div className="space-y-12">
            {favoriteItems.map(item => {
              const video = MOCK_VIDEOS.find(v => v.id === item.videoId);
              const analysis = item.analysis ? JSON.parse(item.analysis) : null;
              if (!video || !analysis) return null;

              return (
                <motion.div 
                  key={item.id}
                  initial={{ opacity: 0, scale: 0.95 }}
                  animate={{ opacity: 1, scale: 1 }}
                  className="vintage-card p-6 rotate-[-0.3deg]"
                >
                  <div className="flex justify-between items-start mb-6">
                    <h3 className="text-2xl font-hand font-bold text-vintage-orange flex items-center gap-3 underline decoration-vintage-orange/10 decoration-4 underline-offset-8">
                      <Utensils className="w-7 h-7" />
                      {analysis.modifiedTitle}
                    </h3>
                    <button 
                      onClick={() => toggleFavorite(item.id)}
                      className="p-2 text-vintage-brown/20 hover:text-red-500 transition-colors"
                    >
                      <Trash2 className="w-7 h-7" />
                    </button>
                  </div>
                  
                  <div className="bg-white/40 p-6 rounded-2xl mb-8 border-2 border-vintage-brown/5">
                    <div className="bg-vintage-cream/60 p-4 rounded-xl mb-8 border-l-4 border-vintage-orange italic font-retro text-lg text-vintage-brown/80 leading-relaxed shadow-inner">
                      “{analysis.healthReason}”
                    </div>
                    
                    <div className="space-y-8">
                      <div>
                        <p className="font-hand font-bold text-vintage-brown text-xl mb-4">
                          准备食材：
                        </p>
                        <div className="flex flex-wrap gap-2">
                          {analysis.ingredients.map((ing: string) => (
                            <span key={ing} className="px-3 py-1 bg-white border border-vintage-brown/10 rounded-full font-retro text-vintage-brown/60">
                              {ing}
                            </span>
                          ))}
                        </div>
                      </div>

                      <div>
                        <p className="font-hand font-bold text-vintage-brown text-xl mb-4">
                          烹饪小妙招：
                        </p>
                        <div className="space-y-4">
                          {analysis.steps.map((step: string, idx: number) => (
                            <div key={idx} className="flex gap-4 items-start">
                              <span className="flex-shrink-0 w-7 h-7 bg-vintage-brown text-vintage-cream rounded-full flex items-center justify-center text-sm font-retro font-bold mt-1">
                                {idx + 1}
                              </span>
                              <p className="text-vintage-brown font-retro text-lg leading-snug">{step}</p>
                            </div>
                          ))}
                        </div>
                      </div>
                    </div>
                  </div>

                  <button className="vintage-btn-primary w-full">
                    <Heart className="w-7 h-7 fill-white" />
                    今天就吃这个
                  </button>
                </motion.div>
              );
            })}
          </div>
        )}
      </main>
    </div>
  );
}
